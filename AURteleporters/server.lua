		...
		if ( getElementType( hitElement ) == "player") and ( mathchingDimension ) then
			...
			if ( z-3 < intMarkers[markerID][3] ) and ( z+3 > intMarkers[markerID][3] ) then
				if not ( getPedOccupiedVehicle( hitElement ) ) then
					if (intMarkers[markerID][12] == "All") or ( ( getPlayerTeam( hitElement ) ) and ( getTeamName( getPlayerTeam( hitElement ) ) == "GIGN" ) or ( getTeamName( getPlayerTeam( hitElement ) ) == "Military Forces" ) or ( getTeamName( getPlayerTeam( hitElement ) ) == "Government Agency" ) or ( getTeamName( getPlayerTeam( hitElement ) ) == "Staff" ) or ( getTeamName( getPlayerTeam( hitElement ) ) == intMarkers[markerID][13] ) ) or ( exports.server:getPlayerGroupName( hitElement ) ) and ( exports.server:getPlayerGroupName( hitElement ) == intMarkers[markerID][12] ) or getElementData( hitElement, "aName" ) == intMarkers[markerID][14] or ( exports.server:getPlayerGroupName( hitElement ) == "FBI" ) then
						...
						triggerClientEvent(hitElement, "setSpawnRotation", hitElement,intMarkers[markerID][9])
